...
void main() {
    if ((texelPos.x * 2 <= shadowMapResolution) || (texelPos.y * 2 <= shadowMapResolution) || DISABLE_SHADOWMAP_BLUR) {
        ...
        for (int i = 1; i < 3; i++) {
            outColor0 += texture(shadowcolor0, texCoord + vec2(offset[i], 0.0f) * texelSize) * weight[i];
            outColor0 += texture(shadowcolor0, texCoord - vec2(offset[i], 0.0f) * texelSize) * weight[i];
        }
        outColor1 = texture(shadowcolor1, texCoord) * weight[0];
        for (int i = 1; i < 3; i++) {
            outColor1 += texture(shadowcolor1, texCoord + vec2(offset[i], 0.0f) * texelSize) * weight[i];
            outColor1 += texture(shadowcolor1, texCoord - vec2(offset[i], 0.0f) * texelSize) * weight[i];
        }
    }
}
